...
class ExtractorBase(ABC):
    ...
    def create_records(self, record_path, wav_files, wav_labels):
        ...
        for n, (wav_file, wav_label) in enumerate(zip(wav_files, wav_labels)):
            tf.logging.info(
                f'[{n}/{N}] Extracting VGGish feature: label: {wav_label} - {wav_file}'
            )
            ...
            if NUM_VGGISH_FEATURE_PER_EXAMPLE > 1:
                if NUM_VGGISH_FEATURE_PER_EXAMPLE != num_features:
                    tf.logging.warning(
                        f'Invalid vggish features length: label: {wav_label} - {wav_file}'
                    )
                    continue
                ...
                if num_features == 0:
                    tf.logging.warning(f'No vggish features: label: {wav_label} - {wav_file}')
                    continue
...
class MelExtractor(ExtractorBase):
    ...
    def wavfile_to_features(wav_file):
        assert os.path.exists(wav_file), f'{wav_file} not exists!'
...
class VGGishExtractor(ExtractorBase):
    ...
    def mel_to_vggish(self, mel_features):
        ...
        return self.postprocess.postprocess(embedding_batch)
